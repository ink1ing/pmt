...
- 可选生成本地建议报告，默认保存到 `~/Documents/PMT/advice.md`。
- 可选通过 Telegram Bot 推送最终建议摘要。
- 支持中英文界面切换。
...
启用建议生成后，PMT 只记录 PMT 改写或语音听写流程中处理过的文本。历史记录保存在本机：

```txt
~/Library/Application Support/PMT/advice-history.jsonl
```

建议生成默认关闭；关闭时不会创建建议历史。启用后，PMT 会读取本地历史，并将生成建议所需的最近片段发送给用户当前配置的模型服务，然后把 100 字以内的短建议写入：

```txt
~/Documents/PMT/advice.md
```

Telegram 是可选推送出口，只发送最终建议摘要，不发送原始本地历史。
